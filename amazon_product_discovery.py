...
def main():
    print(f"🤖 아마존 {TARGET_CATEGORY} 상품 자동 발굴 시작...")
    ...
    for i in range(new_count):
        az_data["products"].append({
            "id": f"az_{datetime.now().strftime('%Y%m%d%H%M%S')}_{i}",
            "name": f"아마존 뷰티·스킨케어 후보 ({random.randint(100,999)})",
            "category": TARGET_CATEGORY,
            "focus_keywords": PLATFORM_KEYWORDS["amazon"],
            "discovered_at": datetime.utcnow().isoformat() + "Z"
        })

    az_data["target_category"] = TARGET_CATEGORY
    az_data["focus_keywords"] = PLATFORM_KEYWORDS["amazon"]
...
def update_cumulative(count, source, new_count):
    ...
    log_entry(f"아마존 뷰티·스킨케어 상품 {new_count}개 발굴 (누적: {total}개)")
