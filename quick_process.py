...
try:
    ...
    transparent_pixels = np.sum(data_check[:,:,3] == 0)
    total_pixels = data_check.shape[0] * data_check.shape[1]
    transparency_percent = (transparent_pixels / total_pixels) * 100

    print(f"\n처리된 이미지 정보:")
    print(f"  - 투명한 픽셀: {transparent_pixels:,}개 ({transparency_percent:.1f}%)")
    print(f"  - 불투명한 픽셀: {total_pixels - transparent_pixels:,}개 ({100-transparency_percent:.1f}%)")
