...
- 실제 TypeSafe 호출은 TYPESAFE_ENABLED=1 과 TYPESAFE_ALLOW_PAID=1 이
  동시에 있어야 한다. API 키만 있다고 호출하지 않는다.
...
def evaluate(context: dict[str, Any]) -> dict[str, Any]:
    ...
    if not _truth("TYPESAFE_ENABLED"):
        ...
        return fallback
    if not _truth("TYPESAFE_ALLOW_PAID"):
        fallback["mode"] = "blocked_no_paid_approval"
        fallback["findings"].append("유료 호출 승인 없음: 로컬 advisory만 사용")
        return fallback
