...
# 카드 시각이 대시보드 생성 시각보다 이만큼 이상 뒤처지면 지적한다.
# 하루 두 번만 도는 고시·비전 같은 건 예외로 둔다.
STALE_HOURS = 3.0
STALE_EXEMPT = {"기관 수집팀", "지식 수집팀", "로보틱스 수집", "디자인팀",
                "옵시디언 그래프"}
...
def load(path: Path):
    try:
        ...
        problems.append(f"{path.name} 을 읽지 못함 ({type(e).__name__})")
        return None
...
def main() -> int:
    ...
    if gen_dt:
        for t in teams:
            ...
            if gap > STALE_HOURS:
                problems.append(
                    f"{t.get('name')} 카드가 {gap:.1f}시간 뒤처짐 "
                    f"(카드 {str(t['when'])[:16]} vs 대시보드 {gen[:16]}). "
                    "워크플로에서 이 팀의 생산 단계가 대시보드보다 뒤에 있거나 "
                    "실패했을 수 있다")
    ...
    if sc:
        ...
        s_card = num(c.get("summary", ""), r"S\s*(\d+)")
        n_card = num(c.get("summary", ""), r"(\d+)개 상품")
        if s_card is not None and s_real is not None and s_card != s_real:
            problems.append(f"소싱 카드 S {s_card} vs 실제 {s_real}")
        if n_card is not None and n_real is not None and n_card != n_real:
            problems.append(f"소싱 카드 상품수 {n_card} vs 실제 {n_real}")
    ...
    if mt and s_real is not None:
        ...
        c = card(teams, "market")
        s_card = num(c.get("summary", ""), r"S등급\s*(\d+)")
        if s_card is not None and s_card != s_real:
            problems.append(f"마케팅 카드 S등급 {s_card} vs 소싱 점수 S {s_real}")

    # ── 3. 리스팅 게이트 ─────────────────────────────────────
    gate = load(D / "listing_gate.json")
    if gate:
        c = card(teams, "listing")
        r_card = num(c.get("summary", ""), r"등록 가능\s*(\d+)")
        t_card = num(c.get("summary", ""), r"등록 가능\s*\d+/(\d+)")
        if r_card is not None and r_card != gate.get("ready"):
            problems.append(f"리스팅 카드 ready {r_card} vs 게이트 {gate.get('ready')}")
        if t_card is not None and t_card != gate.get("total"):
            problems.append(f"리스팅 카드 total {t_card} vs 게이트 {gate.get('total')}")
        if s_real is not None and gate.get("total") not in (None, s_real):
            problems.append(
                f"게이트 대상 {gate.get('total')}건 vs S등급 {s_real}건. "
                "게이트가 점수보다 먼저 돌았다")
        if gate.get("unreadable"):
            problems.append(f"게이트가 읽지 못한 파일: {gate['unreadable']}")

    # ── 4. 채널 ─────────────────────────────────────────────
    gcs = rt.get("global_channels_status") or {}
    live = sum(1 for v in gcs.values() if (v or {}).get("status") == "ok")
    c = card(teams, "channels")
    l_card = num(c.get("summary", ""), r"가동\s*(\d+)")
    t_card = num(c.get("summary", ""), r"가동\s*\d+/(\d+)")
    if gcs:
        ...
        if t_card is not None and t_card != len(gcs):
            problems.append(f"채널 카드 총 {t_card} vs 실제 {len(gcs)}")
    elif l_card:
        problems.append("채널 카드에 숫자가 있는데 global_channels_status 가 비었다")

    cand = load(D / "channel_candidates.json")
    if cand:
        fresh = [x for x in (cand.get("candidates") or [])
                 if x.get("verdict") == "가능" and not x.get("already_live")]
        p_card = num(c.get("summary", ""), r"미연동\s*(\d+)")
        if p_card is not None and p_card != len(fresh):
            problems.append(f"채널 카드 미연동 {p_card} vs 실제 신규 후보 {len(fresh)}")

    # ── 5. 법률: 자동이 사람 판정을 뒤집지 않았나 ────────────
    lp = load(D / "legal_products.json")
    if lp:
        items = lp.get("items") or {}
        hard = [k for k, v in items.items() if v.get("hard_block")]
        for pd_no in hard:
            g = (gate or {}).get("items") or []
            for row in g:
                if str(row.get("pd_no")) == pd_no and row.get("listing_ready"):
                    problems.append(
                        f"{pd_no} 는 법률 hard_block 인데 게이트가 등록 가능으로 뒀다")
        for k, v in items.items():
            if v.get("status") == "fail":
                for row in (gate or {}).get("items") or []:
                    if str(row.get("pd_no")) == k and row.get("legal_ok"):
                        problems.append(f"{k} 는 사람이 fail 로 적었는데 legal_ok 다")

    # ── 6. 소싱 제외 규칙이 실제로 지켜졌나 ──────────────────
    cmap = load(ROOT / "scripts" / "daiso" / "category_map.json")
    prods = load(D / "daiso_real" / "products.json")
    if cmap and prods:
        rows = prods if isinstance(prods, list) else (prods.get("products") or [])
        ex = cmap.get("exclude") or {}
        leaked = []
        for r in rows:
            hay = " ".join(filter(None, [r.get("site_category"), r.get("name")])).lower()
            for name, spec in ex.items():
                if name.startswith("_"):
                    continue
                if any(str(k).lower() in hay for k in (spec.get("keywords") or [])):
                    leaked.append((name, str(r.get("name"))[:24]))
                    break
        if leaked:
            problems.append(
                f"제외 대상이 상품 목록에 {len(leaked)}건 남아 있다: "
                + ", ".join(f"{a}/{b}" for a, b in leaked[:3]))
        else:
            notes.append(f"제외 규칙 준수 확인 ({len(rows)}건 전수)")

    # ── 결과 ────────────────────────────────────────────────
    print(f"대시보드 생성 {gen[:19]} · 팀 {len(teams)}개")
    for n in notes:
        print(f"  OK   {n}")
    if not problems:
        print("  OK   팀 보고 숫자와 원본 파일이 모두 일치")
        return 0
    print(f"\n불일치 {len(problems)}건")
    for p in problems:
        print(f"  FAIL {p}")
