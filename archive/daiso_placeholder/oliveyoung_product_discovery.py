...
def main():
    print(f"🤖 올리브영 {TARGET_CATEGORY} 상품 자동 발굴 시작...")
    ...
    for i in range(new_count):
        oy_data["products"].append({
            "id": f"oy_{datetime.now().strftime('%Y%m%d%H%M%S')}_{i}",
            "name": f"올리브영 뷰티·스킨케어 후보 ({random.randint(100,999)})",
            "category": TARGET_CATEGORY,
            "focus_keywords": PLATFORM_KEYWORDS["oliveyoung"],
            "discovered_at": datetime.utcnow().isoformat() + "Z"
        })

    oy_data["target_category"] = TARGET_CATEGORY
    oy_data["focus_keywords"] = PLATFORM_KEYWORDS["oliveyoung"]
...
def update_cumulative(count, source, new_count):
    ...
    log_entry(f"올리브영 뷰티·스킨케어 상품 {new_count}개 발굴 (누적: {total}개)")
